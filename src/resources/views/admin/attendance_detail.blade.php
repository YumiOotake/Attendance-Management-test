...
@section('content')
    <div class="request-form__wrapper">
        <div class="attendance__inner">
            ...
            @else
                <form action="{{ route('admin.request', $attendance->id ?? 0) }}?date={{ $attendanceDate }}&user_id={{ $attendanceUser->id }}&from={{ $from }}&month={{ $monthOffset }}" method="POST" class="request-form">
                    @csrf
                    @method('PATCH')
                    <div class="request-form__group">
                    ...
                    <div class="request-form__group">
                        ...
                        <div class="request-form__content">
                            <input type="text" id="requested_break_start[]" name="requested_break_start[]" value="{{ old('requested_break_start.' . count($breaks))}}"
                                class="request-form__input">
                            <span class="request-form__span">〜</span>
